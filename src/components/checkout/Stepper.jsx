...
export default function Stepper({ steps = [], current = 0, onJump }) {
  return (
    <Box as="ol" listStyleType="none" m={0} p={0} display="grid" gridTemplateColumns={`repeat(${steps.length}, 1fr)`} gap={2} aria-label="Checkout progress">
      {steps.map((s, i) => {
          ...
          <Box as="li" key={s} aria-current={active ? 'step' : undefined}>
            ...
            >
              ...
              <HStack spacing={2}>
                ...
                <Text fontFamily="heading" fontWeight={700} textTransform="uppercase" letterSpacing="0.08em" fontSize={{ base: 'xs', md: 'sm' }} color={active ? 'bone.100' : done ? 'bone.300' : 'bone.600'} noOfLines={1}>{s}</Text>
              </HStack>
            </Box>
          </Box>
        )
